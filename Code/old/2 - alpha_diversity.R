...
tidy.gls <- function(gls_mod){
  the_anova <- anova(gls_mod)
  as_tibble(the_anova, rownames = 'term') %>%
    filter(term != '(Intercept)') %>%
    select(-term) %>%
    rename(f.value = 'F-value',
           p.value = 'p-value') %>%
    mutate(denDF = str_extract(attr(the_anova, 'label'), '[0-9]+') %>% as.integer())
...
postDose_effects <- alpha_diversity %>%
  ...
  nest_by(alpha_metric) %>%
  reframe(treatment_model = list(gls(value ~ treatment, 
                                     data = data,
                                     weights = varIdent(form = ~1|treatment))),
          tidy.gls(treatment_model),
          
          posthoc = list(emmeans(treatment_model, ~treatment, data = data)),
...
# ttest <- predose_effects$ttest[[1]]
# posthoc <- postDose_effects$posthoc[[1]]
# contrast <- postDose_effects$planned_contrast[[1]]

plotData_joint_alpha <- function(ttest, posthoc){
  posthoc %>%
    cld(Letters = LETTERS) %>%
    broom::tidy() %>%
    mutate(time = 'after',
    ...
    bind_rows(tibble(anti = c('N', 'A'),
                     ...
                     std.error = ttest$stderr_pair,
                     simple_sig = if_else(ttest$p.value < alpha, '*', ''),
